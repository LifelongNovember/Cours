...
$$ X_i \sim \mathcal B(1 , \pi) $$

$$ \overline X = {{\sum_{i = 1}^n X_i} \over n} $$

$$ \sum_{i = 1}^n X_i \sim \mathcal B ( n , n \pi) $$

$$ n \geqslant 30 \ : \ \overline X \sim \mathcal N ( \pi , \sqrt {\pi ( 1 - \pi) / n} ) $$

$$ {{\overline X - \pi} \over \sqrt {\pi(1 - \pi) / n}} \sim \mathcal N ( 0,1) $$

$$ P \left ( \left | {{ \overline X -\pi} \over \sqrt {\pi(1 - \pi) / n}} \right | \leqslant u_{1 - \alpha / 2} \right ) = 1 - \alpha $$


$$ \begin{gathered} 
\left | {{\overline X - \pi} \over \sqrt{\pi (1 - \pi) / n}} \right | \leqslant u_{1 - \alpha/2} \\
{{(\overline X - \pi)^2} \over {\pi ( 1 - \pi) / n}} \leqslant u_{1 - \alpha/2} ^2 \\
(\overline X - \pi)^2 \leqslant u_{1 - \alpha/2} \times {{\pi (1 - \pi)} \over n } \\
\overline X^2 - 2 \overline X \pi + \pi^2 \leqslant u_{1 - \alpha/2}^2 \times {\pi \over n} - u_{1 - \alpha/2}^2 \times {\pi^2 \over n} \\
...
\left ( 1 + {1,96^2 \over 40} \right ) \pi^2 - \left ( 2 \times 0,25 + {1,96^2 \over 40} \right ) \pi + 0,25^2 \\
1,096 \pi^2 - 0,596 \pi + 0,0625 \\ ~ \\
\Delta = 0,596^2 - 4  \times 1,096 \times 0,0625 \approx 0,08125 \\
\pi_{1/2} = {{0,596 +/- \sqrt {0,08125} } \over {2 \times 1,096}}  = {{0,1418} \atop {0,402} } \\ ~ \\
...
$$ \begin{gathered}
\left | {{\overline X - \pi} \over \sqrt {\overline X (1 - \overline X) / n }} \right | \leqslant u_{1 - \alpha/2} \\
-u_{1 - \alpha / 2} \leqslant {{\overline X - \pi} \over \sqrt {\overline X (1 - \overline X) / n }} \leqslant u_{1 - \alpha / 2} \\
\overline X - u_{1 - \alpha/2} \sqrt { \overline X (1 - \overline X ) / n} \leqslant  \pi \leqslant \overline X + u_{1 - \alpha/2} \sqrt { \overline X (1 - \overline X ) / n}  \\
I = \left [ \overline X - u_{1 - \alpha/2} \sqrt { \overline X (1 - \overline X ) / n}  ; \overline X + u_{1 - \alpha/2} \sqrt { \overline X (1 - \overline X ) / n}  \right ] \\
 = [11,6 \% ; 38,4 \%] : 25\% \text{ à 13,4\% près}
\end{gathered} $$
...
I = \left [ \overline X - u_{1 - \alpha/2} {1 \over {2\sqrt n}} \ ; \ \overline X + u_{1 - \alpha/2} {1 \over {2\sqrt n}} \right ] \\
I = [ 9,50 \% ; 40,50 \% ] : 25 \text{ à 15,5\% près}
\end{gathered} $$

Remarque : $$ \overline X \text{ à } 2 \times {1 \over {2\sqrt n}} \text{ càd } {1 \over \sqrt n} \text{ près : } \left [ \overline X - {1 \over \sqrt n} ; \overline X + {1 \over \sqrt n} \right ]$$
$$ \implies {1 \over \sqrt {40} } \approx 15,\% $$

---

$$ 1,0038 \pi ^2 - 0,5038 \pi + 0,0625 $$ $$ \Delta \approx 0,00286 $$ $$ I = [ 22,4 \% ; 27,8 \% ] : 25,8\% \text{ à 2,7\% près} $$
...
$$ X \sim \mathcal B (20, \pi) $$ $$ P(X \geqslant 18) \geqslant 0,05 $$ $$ 1 - P(X \leqslant 17 ) \geqslant 0,05 $$ $$ 1 - P(X \leqslant 17 ) \leqslant 0,95 $$

$$ X' \sim \mathcal B (20, 1 - \pi ) $$ $$ P(X \geqslant 18) = P(X' \leqslant  20 - 18) = P(X' \leqslant 2) $$

Table : $$ P(X' \leqslant 2) = 0,0355 < 0,05 \text{ pour $p' = 0,300$ } $$ $$ P(X' \leqslant 2) = 0,0913 \geqslant 0,05 \text { pour $p' = 0,250$} $$ $$\implies IC = [p_1 ; 1 ] \text{ avec $ 0,7 < p_1 < 0,75$} $$

Abaque : $p_1 \approx 0,72$

Représentation graphique : $p_1 \approx 0,72$

Programme : $p_1 \approx 0,7174$
...
$$ \begin{gathered} P_c( \overline x) = P_{H_0} ( \left | \overline X - \mu_0 \right | > \left | \overline x - \mu_0 \right | )  \\ = P_{H_0} ( | \overline X - \mu_0 | > 3) \\ = P_{H_0} (\overline X - \mu_0 > 3 ) + P_{H_0}( \overline X - \mu_0 < - 3) \\ = P_{H_0} ( \overline X > 51) + P_{H_0} ( \overline X < 45) = 2 P_{H_0} ( \overline X > 51) \end{gathered} \\ 2 P_{H_0} ( T > {{51 - 48} \over {5 / \sqrt {25}}}) = 2 P_{H_0} (T> 3) \\ = 2(1 - 0,9987) = 0,26 \% < 5 \%  \implies \text{ rejet de } H_0$$

Test très significatif.

## Exercice 18 - Test pour une moyenne ($\sigma$ inconnu)

$$ X \sim \mathcal N( \mu, \sigma) \quad \mu_0 = 250g$$

$$ H_0 : \mu = \mu_0 \quad H_1 : \mu < \mu_0 $$

$$ \alpha = 1\% $$

$$ T = {{\overline X - \mu_0} \over {S / \sqrt n}} \sim \mathcal T_{n-1} $$

$$ \overline X = {1 \over n} \sum_{i = 1} ^n x_i \quad S^2 = {1 \over {n - 1}} \sum_{n = 1} ^n (X_i - \overline X)^2 $$
...
$$R_1 = [2,821, + \infty[ $$
$$\begin{aligned}
R_2 : {{\overline X - \mu_0} \over {S / \sqrt n}} \leqslant -2,821 \\
...
On ne peut pas rejeter l'hypothèse nulle $H_0 : \mu = 250$.

Remarque : ${{\overline x - 250} \over {1,395 / \sqrt{10}}} { ? \atop \in} R_1$ : $-1,904 \notin R_1$.
...
Remarque : au niveau 5%, rejet de $H_0$.
...
$$H_0 : \pi = 60 \% \quad H_1 : \pi \neq 60\%$$ $$X_i \sim \mathcal B(1 \ ; 0,6)$$ $$X = X_1 + \dots + X_{18} \sim \mathcal B(18 \ ; 0,6)$$ $$R = [\![0 \ ; n]\!] \cup [\![n_2 \ ; 18]\!]$$

$n_1$ le plus grand tel que : $P(X \leqslant i) \leqslant 0,025$.
$n_2$ le plus petit tel que : $P(X \geqslant i) \leqslant 0,025$.
...
Le plus petit $17 - i$ tel que $P(X' \leqslant 17 - i) \geqslant 0,975$ :  $17 - i = 11 \ ; i = 17 - 11 = 6$.

$$ P(X' \leqslant 18 - i) \leqslant 0,025$$
Le plus grand $18 - i$ tel que $P(X' \leqslant 18 - i ) \leqslant  0,025$ : $18 - i = 2 \ ; i = 16$.

$$R = [\![ 0 \ ; 6 ]\!] \cup [\![ 16 \ ; 18]\!]$$
$$5 \notin R \quad \text {rejet de $H_0 : \pi = 60\%$}$$

$$ P_C(5) = p_1 + p_2 $$ $$ p_1 = P(X \leqslant 5) $$ 
$$\begin{gathered}
p_2 : P(X = 5) = 0,0044 \\
...
$$ D = \sum_0 ^9 {{(N_i - n \pi)^2} \over {n \pi}} = {{(11 - 15)^2} \over 15 } + ... + {{(18 - 15)^2} \over 15} $$ $$ D \sim \chi _{m - r - 1} ^2 \quad m : \text{nb de valeurs} \quad r : \text{nb param. estimés}$$ $$D \sim \chi_{10 - 0 - 1} ^2 \sim \chi_9 ^2 $$
...
$\delta$ tel que $P(D \leqslant \delta) = 0,95$ : $\delta = 16,919$.

$$ R = [16,919 \ ; + \infty[ $$ $$ d = 5,067 \notin R$$

p-value : $P_c(d) = P_{H_0} ( D \geqslant d)$ $$ \begin{aligned}
P_c(d) &= P_{H_0} (D \geqslant d)  \\
&= 1 - P_{H_0} (D \leqslant d) \\
&= 1 - P_{H_0} (D \leqslant 5,06)
\end{aligned} $$ $$ P(D \leqslant 4,507) \leqslant P(D \leqslant 5,06) \leqslant P(D \leqslant 5,38) $$ $$ \begin{gathered}

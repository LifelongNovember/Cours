...
#### Expérience de Weldon

On lance $n$ fois un dé à 6 faces, on s'intéresse à la réalisation de $A = \{5,6\} \included \Omega = \{1,2,3,4,5,6\}$.

À chaque lancer, la variable aléatoire $X_i$, $(1 \leqslant i \leqslant n)$ prend la valeur 1 si $\omega \in A$, 0 sinon.

$X_i \sim \mathcal B(1, \pi)$ et on suppose que $\overline X = {{X_1 + \dots + X_n} \over n} \sim \mathcal N ( \pi, \sqrt {\pi (1 - \pi)/n})$ CF 62

La région de rejet contient les valeurs les plus improbables de l'ensemble des valeurs possibles de la variable (ici : $\mathcal R = \{ \overline x, \left | \overline x - 1/3 \right | \geqslant 0,1 \}$).

La variable aléatoire $\overline X$ (ou l'aménagement fait sur celle-ci) est dite **statistique de test** (notée $T$).
De manière générale, $T$ est associée à un estimateur du paramètre $\theta$. La **règle de décision** s'écrit alors  $T \in \mathcal R$ et on rejette $H_0$.

CF test unilatéral / bilatéral (66/67)

$H_0 : \theta \in I_0$ ($I_0$ intervalle), $I_1 = \overline {I_0}$.
**Risque de première espèce** : $\alpha(\theta) = P_{\theta \in I_0} (T \in \mathcal R)$, probabilité de rejeter $H_0$, alors qu'elle est vraie.

**Niveau** : $\alpha = {\sup _{\theta \in I_0}} \alpha(\theta)$

**Risque de deuxième espèce** : CF 69

Remarques : CF 70 - 73

#### p-value

Probabilité d'avoir pire que le résultat trouvé.
CF 77
La **signification statistique** d'un test est mesuré par la **p-value** :
CF 79
Le test est d'autant plus **significatif** que la p-value est proche de 0.

Pour rejeter $H_0$, on peut comparer :
 * la valeur $t$ observée ou calculée et la (les) valeur(s) seuil(s) CF 81
 * 
...
$$ \begin{aligned}
\chi_{n-1;\alpha/2}  \leqslant (n - 1) {S^2 \over \sigma^2} \leqslant \chi_{n - 1;1 - \alpha/2} &\iff {\chi_{n - 1;\alpha/2} \over {n - 1}} \leqslant {S^2 \over \sigma^2} \leqslant {\chi_{n - 1;1 - \alpha/2} \over {n - 1}} \\ ~ \\
&\iff {\chi_{n - 1;\alpha/2} \over {(n - 1) S^2}} \leqslant {1 \over \sigma^2} \leqslant {\chi_{n - 1; 1 - \alpha/2} \over {(n - 1) S^2}} \\ ~ \\
&\iff {{(n - 1) S^2} \over {\chi_{n - 1; \alpha/2}}} \geqslant \sigma^2 \geqslant  {{(n - 1) S^2} \over \chi_{n - 1; 1 - \alpha/2}} \\ ~ \\
&\iff \sqrt {{{(n - 1) S^2} \over {\chi_{n - 1; \alpha/2}}}} \geqslant  \sigma \geqslant \sqrt{ {{(n - 1) S^2} \over \chi_{n - 1; 1 - \alpha/2}} } \\ ~ \\
&\iff 25,55 \geqslant \sigma \geqslant 3,88
\end{aligned} $$ $$ I = [3,88 \ ; \ 25,55] $$

![](https://i.imgur.com/vLAnqiZ.png)

## Exercice 15 - Intervalle de confiance pour une proportion

> Dans une population donnée, on interroge $n$ personnes sur leur intention de vote.

> ### $n = 40$

> 10 personnes envisagent voter pour le candidat A.

> #### Donner une estimation de la proportion $\pi$ de votants dans la population.

$$ f_{40} = {10 \over 40} = {1 \over 4} \quad \text{(estimation de $\pi$)} $$

> #### Donner un intervalle de confiance pour $\pi$ de seuil $5\%$ (ou de niveau de confiance $95\%$)

> ##### en prenant $\overline X \sim \mathcal N ( \pi , \sqrt {\pi ( 1 - \pi) / n})$,
> 
> > $P \left ( \left | {{\overline X - \pi} \over {\sqrt {\pi ( 1 - \pi) / n}}} \right | \leqslant u_{1 - \alpha/2} \right ) = 1 - \alpha$ conduit à ${{(\overline X - \pi)^2} \over {\pi ( 1 - \pi) / n}} \leqslant u_{1 - \alpha/2} ^2$ puis à $\left ( 1 + {u_{1 - \alpha/2} ^2 \over n} \right ) \pi^2 - \left ( 2 \overline X + {u_{1 - \alpha/2} ^2 \over n} \right ) \pi + \overline X ^2 \leqslant 0$.

$$ X_i \sim \mathcal B(1 , \pi) :  \overline X = {{\sum_{i = 1}^n X_i} \over n} \implies \sum_{i = 1}^n X_i \sim \mathcal B ( n , n \pi) $$

$$ n \geqslant 30 \ : \ \overline X \sim \mathcal N ( \pi , \sqrt {\pi ( 1 - \pi) / n} ) \implies {{\overline X - \pi} \over \sqrt {\pi(1 - \pi) / n}} \sim \mathcal N ( 0,1) $$

$$ \footnotesize \begin{aligned}
P \left ( \left | {{ \overline X -\pi} \over \sqrt {\pi(1 - \pi) / n}} \right | \leqslant u_{1 - \alpha / 2} \right ) = 1 - \alpha  &\iff \left | {{\overline X - \pi} \over \sqrt{\pi (1 - \pi) / n}} \right | \leqslant u_{1 - \alpha/2} \\
&\iff {{(\overline X - \pi)^2} \over {\pi ( 1 - \pi) / n}} \leqslant u_{1 - \alpha/2} ^2 \\
&\iff \overline X^2 - 2 \overline X \pi + \pi^2 \leqslant u_{1 - \alpha/2}^2 \times {\pi \over n} - u_{1 - \alpha/2}^2 \times {\pi^2 \over n} \\
&\iff 
\left ( 1 + {u_{1 - \alpha/2}^2 \over n} \right ) \pi^2 - \left ( 2 \overline X + {u_{1 - \alpha/2}^2 \over n} \right ) \pi + \overline X^2 \leqslant 0 
\end{aligned}$$


![](https://i.imgur.com/VyFlRay.png)

$$ \alpha = 5\% \ : \ u_{1 - 0,05/2} = u_{0,975} \approx 1,96 $$

$$ \footnotesize \begin{aligned}
P \left ( \left | {{ \overline X -\pi} \over \sqrt {\pi(1 - \pi) / n}} \right | \leqslant u_{1 - \alpha / 2} \right ) = 1 - \alpha &\iff \left ( 1 + {1,96^2 \over 40} \right ) \pi^2 - \left ( 2 \times 0,25 + {1,96^2 \over 40} \right ) \pi + 0,25^2 \leqslant 0 \\
&\iff 1,096 \pi^2 - 0,596 \pi + 0,0625
\end{aligned} $$

$$ \Delta = 0,596^2 - 4  \times 1,096 \times 0,0625 \approx 0,08125 \\ ~ \\
\pi_{1 \atop 2} = {{0,596 +/- \sqrt {0,08125} } \over {2 \times 1,096}}  = \left \{ {{0,142} \atop {0,402} } \right. \\ ~ \\
I = [ 0,142 ; 0,402 ] = [ 14,2\% ; 40,2 \% ] : 27,2\% \text{ à $13\%$ près} $$

> ##### en prenant  ${{\overline X - \pi} \over \sqrt{\overline X (1 - \overline X) / n}} \sim \mathcal N (0,1)$,
> > $$ P \left ( \left | {{\overline X - \pi} \over \sqrt{\overline X (1 - \overline X) / n}} \right | \leqslant u_{1 - \alpha / 2} \right ) = 1 - \alpha $$

$$ {{\overline X - \pi} \over \sqrt {\overline X (1 - \overline X) / n }}  \sim \mathcal N(0,1) \ : \ P \left ( \left | {{\overline X - \pi} \over \sqrt {\overline X (1 - \overline X) / n }} \right | \leqslant u_{1 - \alpha/2} \right ) = 1 - \alpha $$

$$ \footnotesize \begin{aligned}
\left | {{\overline X - \pi} \over \sqrt {\overline X (1 - \overline X) / n }} \right | \leqslant u_{1 - \alpha/2} &\iff -u_{1 - \alpha / 2} \leqslant {{\overline X - \pi} \over \sqrt {\overline X (1 - \overline X) / n }} \leqslant u_{1 - \alpha / 2} \\
&\iff \overline X - u_{1 - \alpha/2} \sqrt { \overline X (1 - \overline X ) / n} \leqslant  \pi \leqslant \overline X + u_{1 - \alpha/2} \sqrt { \overline X (1 - \overline X ) / n}  \\
\end{aligned} $$


$$ \begin{aligned}
I' &= \left [ \overline X - u_{1 - \alpha/2} \sqrt { \overline X (1 - \overline X ) / n}  \ ; \  \overline X + u_{1 - \alpha/2} \sqrt { \overline X (1 - \overline X ) / n}  \right ]  \\ &= [11,6 \% \ ; \ 38,4 \%] : 25\% \text{ à 13,4\% près} 
\end{aligned}$$

> ##### en majorant $\pi (1 - \pi)$ par $1 \over 4$ dans l'approximation précédente.

Approximation : $x (1 - x) \leqslant  1/4 \text { sur [0,1]} \ : \ \sqrt{x (1 - x) } \leqslant 1/2 \text{ sur [0,1]}$

> ### $n = 1000$

> 250 personnes envisagent de voter pour le candidat A.

> #### Donner une estimation de la proportion $\pi$ de votants dans la population.

$$ f_{250} = {250 \over 1000} = {1 \over 4} \quad \text{(estimation de $\pi$)} $$ 

> #### Donner un intervalle de confiance de seuil $5\%$

> ##### en prenant $\overline X \sim \mathcal N (\pi, \sqrt{\pi (1 - \pi)/n})$,

$$ \begin{aligned}
I &= \left [ \overline X - u_{1 - \alpha/2} {{1/2} \over \sqrt n} \ ; \ \overline X + u_{1 - \alpha/2} {{1/2} \over \sqrt n} \right ] \\
&= \left [ \overline X - u_{1 - \alpha/2} {1 \over {2\sqrt n}} \ ; \ \overline X + u_{1 - \alpha/2} {1 \over {2\sqrt n}} \right ] \\
&= [ 9,50 \% ; 40,50 \% ] : 25 \text{ à 15,5\% près}
\end{aligned} $$

Remarque : $\overline X \text{ à } 2 \times {1 \over {2\sqrt n}} \text{ càd } {1 \over \sqrt n} \text{ près : } \left [ \overline X - {1 \over \sqrt n} \ ; \ \overline X + {1 \over \sqrt n} \right ] \implies {1 \over \sqrt {40} } \approx 15,8\%$

---

$$ P \left ( \left | {{ \overline X -\pi} \over \sqrt {\pi(1 - \pi) / n}} \right | \leqslant u_{1 - \alpha / 2} \right ) =  1,0038 \pi ^2 - 0,5038 \pi + 0,0625 $$ $$ \Delta \approx 0,00286 $$ $$ I = [ 22,4 \% ; 27,8 \% ] : 25,8\% \text{ à 2,7\% près} $$

> en prenant ${{\overline X - \pi} \over \sqrt{\overline X (1 - \overline X) / n}} \sim \mathcal N (0,1)$.

$$ I' = [21,9\% ; 28,1 \% ] : \text{25\% à 3,1\% près} $$

## Exercice 16 - Intervalle de confiance unilatéral à droite pour une proportion

> Dans une région, il y a une proportion $\pi$ de trains arrivant à l'heure.
> On prélève un échantillant de $n = 20$ trains de manière aléatoire. Il y a $n_h = 18$ trains arrivant à l'heure.
> Un intervalle de confiance $\pi$ de niveau $1 - \alpha$ unilatéral à droite est l'ensemble des nombres $\pi$ vérifiant $X \sim \mathcal B(20, \pi)$ et $P(X \geqslant 18) \geqslant 0,05$.
> > $n = 20$ ne permet pas une approximation suivant la loi normale.
>
> ### Préciser un intervalle de confiance de $\pi$ de niveau $95\%$ unilatéral à droite $[p_1, 1]$

$$ \begin{aligned}
X \sim \mathcal B (20, \pi) : P(X \geqslant 18) \geqslant 0,05 &\iff 1 - P(X \leqslant 17 ) \geqslant 0,05  \\
&\iff 1 - P(X \leqslant 17 ) \leqslant 0,95
\end{aligned}$$

$$ X' \sim \mathcal B (20, 1 - \pi ) : P(X \geqslant 18) = P(X' \leqslant  20 - 18) = P(X' \leqslant 2) $$

> #### en utilisant une table des probabilités de la loi binomiale,

$$ P(X' \leqslant 2) = 0,0355 < 0,05 \text{ pour $p' = 0,300$ } $$ $$ P(X' \leqslant 2) = 0,0913 \geqslant 0,05 \text { pour $p' = 0,250$} $$ $$\implies IC = [p_1 ; 1 ] \text{ avec $ 0,7 < p_1 < 0,75$} $$

> #### en utilisant un abaque,

 $$p_1 \approx 0,72$$

> #### en utilisant la représentation graphique $P(X \geqslant 18)$ en fonction de $\pi$, 

$$p_1 \approx 0,72$$

> #### en programmant cette dernière fonction.

$$p_1 \approx 0,7174$$

## Exercice 17 - Test pour une moyenne ($\sigma$ connu)

> Dans une usine, le *temps* $X$ en minutes mis par une machine pour fabriquer une pièce est une variable aléatoire qui suit une loi normale $\mathcal N (\mu_0, \sigma)$, avec $\mu_0 = 48$ minutes et $\sigma = 5$ minutes.
> La machine tombe en panne. Elle est remplacée par une autre machine avec laquelle on fabrique la même pièce. Le temps de fabrication en minutes mis par cette machine est une variable aléatoire qui suit une loi normale $\mathcal N (\mu, \sigma)$, avec $\mu$ inconnu et $\sigma = 5$ minutes.
> Pour un échantillon de $n = 25$ pièces provenant de celle-ci, on a obtenu un temps moyen de fabrication de $\overline x = 51$ minutes.
> On se demande si on peut affirmer au niveau $\alpha = 5\%$ que la nouvelle machine n'a pas les mêmes performances que la machine précédente.
> On veut donc tester l'hypothèse nulle $H_0 : \mu = \mu_0$, contre l'hypothèse alternative $H_1 : \mu \neq \mu_0$ au niveau $5\%$.
> En notant $X_i$ le *temps* de fabrication d'une pièce donnée $i$, on peut supposer que $X_i \sim \mathcal N(\mu, \sigma)$ et les $X_i$ indépendantes.

> Sous l'hypothèse nulle $H_0 : \mu = \mu_0$, $T = {{\overline X - \mu_0} \over {\sigma / \sqrt n}} \sim \mathcal N(0,1)$ avec $\displaystyle \overline X = {1 \over n} \sum_{i = 1} ^n X_i$.

> ### Préciser le nombre $u$ vérifiant $P(-u \leqslant T \leqslant u) = 0,95$ avec $T \sim \mathcal N (0,1)$.

$$ T = {{\overline X - \mu_0} \over {\sigma / \sqrt n}} \sim \mathcal (0,1) $$

$$ u = u_{0,975} \qquad \alpha = 5\% $$

> ### En déduire $P(|T| \geqslant u)$, puis la région de rejet.

Si on prend $T \sim \mathcal N (0,1)$, $\mathcal R = ] -\infty \ ; \ -1,96 [ \cup ]1,96 \ ; \ + \infty[$

$$ - 1,96 \leqslant {{\overline X - \mu_0} \over {\sigma / \sqrt n}} \leqslant 1,96 \quad \text{(région d'acceptation)}$$

$$ \begin{gathered}
\mu - 1,96 \times {\sigma / \sqrt n} \leqslant \overline X \leqslant \mu_0 + 1,96 \times {\sigma / \sqrt n} \\
46,04 \leqslant \overline X \leqslant 49,96 \\ ~ \\
\text{Région de rejet : } [0 \ ; \ 46,04[ \cup ] 49,96 \ ; \ + \infty[
\end{gathered} $$

> ### Est-ce que $\overline x$ appartient à la région de rejet ?

Expérience : réalisation de $\overline X$
$$ \overline x = 51 \in \mathcal R $$
En utilisant $T \sim \mathcal N (0,1)$,

$${{\overline x - \mu_0} \over {\sigma / \sqrt n}} = {{51 - 48} \over {5 / \sqrt 25}} = 3 \in ] - \infty ; -1,96[ \cup ]1,96 ; + \infty [ $$

> ### En déduire la décision. Expliquer.

$\overline x$ appartenant à la région de rejet, on prend la décision de rejeter l'hypothèse $H_0$.

> ### Préciser un encadrement de la p-value $P_c (\overline x) = P_{H_0} ( | \overline X - \mu_0 | \geqslant | \overline x - \mu_0 |)$. En déduire la signification du test.

$$ \begin{aligned} P_c( \overline x) &= P_{H_0} ( \left | \overline X - \mu_0 \right | > \left | \overline x - \mu_0 \right | )  \\ &= P_{H_0} ( | \overline X - \mu_0 | > 3) \\ &= P_{H_0} (\overline X - \mu_0 > 3 ) + P_{H_0}( \overline X - \mu_0 < - 3) \\ &= P_{H_0} ( \overline X > 51) + P_{H_0} ( \overline X < 45) = 2 P_{H_0} ( \overline X > 51) \end{aligned} \\ ~ \\\begin{aligned} 2 P_{H_0} ( T > {{51 - 48} \over {5 / \sqrt {25}}}) &= 2 P_{H_0} (T> 3) \\ &= 2(1 - 0,9987) \\ &= 0,26 \% < 5 \%  \implies \text{ rejet de } H_0 \end{aligned} \\  ~ \\ \text{Test très significatif}$$

## Exercice 18 - Test pour une moyenne ($\sigma$ inconnu)

>  Le *poids net* $X$ d'une boîte de gâteaux d'une marque donnée  (en grammes) suit une loi normale $\mathcal N(\mu, \sigma)$ avec $\mu$ et $\sigma$ inconnus.
>  Une association de consommateurs prélève au hasard $n = 10$ boîtes :
>  $$ 249,3 \ ; \ 250,6 \ ; \ 250,7 \ ; \ 250,3 \ ; \ 250,6 \ ; \ 246,7 \ ; \ 248,2 \ ; \ 248,0 \ ; \ 249,2. $$ L'étiquette des boîtes indique un *poids net* de $\mu_0 = 250$ grammes.
>  On veut tester l'hypothèse nulle $H_0 : \mu = \mu_0$, contre l'hypothèse alternative $H_1 : \mu < \mu_0$ au niveau $\alpha = 1\%$.
>  En notant $X_i$, le *poids net* d'une boîte donnée $i$, on peut supposer que $X_i \sim \mathcal N(\mu, \sigma)$ et les $X_i$ indépendantes.
>  Sous l'hypothèse nulle $H_0 : \mu = \mu_0$, $T = {{\overline X - \mu_0} \over {S / \sqrt n}} \sim \mathcal T_{n - 1}$, avec $\displaystyle \overline X = {1 \over n} \sum_{i = 1} ^n X_i$ et $\displaystyle S^2 = {1 \over {n - 1}} \sum _{i = 1} ^n (X_i - \overline X)^2$.

> ### Calculer $\overline x$, $s^2$ et $s$.
...
> ### Préciser le nombre $t_{1 - \alpha}$ vérifiant $P(T \leqslant t_{1 - \alpha}) = 1 - \alpha = 0,99$ avec $T \sim \mathcal T_{n - 1}$.

$$ \begin{gathered}
P( T \leqslant t_{1 - \alpha} ) = 1 - \alpha \quad (\alpha = 1\%) \\
P(T \leqslant t_{0,99}) = 0,99
\end{gathered}$$

![](https://i.imgur.com/mjPTzne.png)
Table : $t_{0,99} = 2,821$

> ### En déduire $P(T \leqslant -t_{1 - \alpha})$, puis la région de rejet. Est-ce que $\overline x$ appartient à la région de rejet ?

$$P(T \leqslant - t_{0,99}) = 0,01 \quad \text{(symétrie)}$$
Région de rejet : $$R_1 = [2,821 \ ; \ + \infty[ $$ $$
R_2 : {{\overline X - \mu_0} \over {S / \sqrt n}} \leqslant -2,821 \iff
\overline X \leqslant \mu_0 - 2,821 {S \over \sqrt n} \\
R_2 = [0 \ ; 248,756] : \overline x \notin R_2 $$

> ### En déduire la décision. Expliquer.
La réalisation n'est pas située dans la région de rejet, on ne peut donc pas rejeter l'hypothèse nulle $H_0 : \mu = 250$.

> ### Préciser un encadrement de la p-value $P_c(\overline x) = P_{H_0} (\overline X \leqslant \overline x)$.


$$\begin{aligned}
P_c(\overline x) &= P_{H_0} ( \overline X \leqslant \overline x) \\
&= P_{H_0} \left ( {{\overline X - \mu_0} \over {S / \sqrt n}} \leqslant  {{\overline x - \mu_0} \over {s / \sqrt n}} \right ) \\
&= P_{H_0} \left ( T \leqslant {{249,16 - 250} \over {1,395 / \sqrt {10}}} \right ) \\
&= P_{H_0} ( T \leqslant -1,904) \\
&= P_{H_0} ( T \geqslant 1,904) \\
&= 1 - P_{H_0} ( T \leqslant 1,904)
\end{aligned}$$

Table : $1,833 < 1,904 < 2,269$ d'où $0,95 < P(T \leqslant 1,904) < 0,975$.

> ### En déduire le degré de signification du test.

$$ \begin{gathered}
1 - 0,975 < P_c (\overline x) < 1 - 0,95 \\
0,025 < P_c ( \overline x) < 0,05 \\
2,5 \% < P_c(\overline x) < 5\%
\end{gathered} \\
\text{Test significatif}$$

> $0,01 < P_c(\overline x) \leqslant 0,05$ : test significatif
> $0,001 < P_c(\overline x) \leqslant 0,01$ : test très significatif
> $P_c(\overline x) \leqslant 0,001$ : test hautement significatif


## Exercice 19 - Test pour une proportion

> Dans une université, on veut tester l'hypothèse : $60\%$ des étudiants pratiquent un sport.
> On interroge $n = 18$ étudiants pris au hasard : 5 étudiants pratiquent un sport.
> On souhaite alors tester l'hypothèse nulle $H_0 : \pi = 0,6$ contre l'hypothèse alternative $H_1 : \pi \neq 0,6$ au niveau $\alpha = 5\%$.
> Soit $X_i \sim \mathcal B(1 \ ; \ 0,6) \ (1 \leqslant i \leqslant 18)$ indépendantes.

> ### Préciser la loi suivie par $X = X_1 + \dots + X_n$ $(n = 18)$.

$$H_0 : \pi = 60 \% \quad H_1 : \pi \neq 60\%$$ $$X_i \sim \mathcal B(1 \ ; 0,6)$$ $$X = X_1 + \dots + X_{18} \sim \mathcal B(18 \ ; 0,6)$$ 

> ### Préciser la région de rejet de $H_0 : \mathcal R = [\![0, n_1]\!] \cup [\![n_2, n]\!]$.
> > * $n_1$ est le plus grand entier $i$ tel que $P(X \leqslant i) \leqslant 0,025$ et $n_2$ le plus petit entier $i$ tel que $P(X \geqslant i) \leqslant 0,025$.
> > * $P(X \leqslant i) = 1 - P(X' \leqslant n - 1 - 1)$ avec $X' \sim \mathcal B(n , 1 - p)$

$$R = [\![0 \ ; n]\!] \cup [\![n_2 \ ; 18]\!]$$



$$\scriptsize P(X \leqslant i ) \leqslant 0,025 \iff 1 - P( X' \leqslant 18 - i - 1) \leqslant 0,025 \ ; X' \sim \mathcal B(18 \ ; 1 - 0,6) $$

$$ P(X' \leqslant 17 - i) \geqslant 0,975$$

Le plus petit $17 - i$ tel que $P(X' \leqslant 17 - i) \geqslant 0,975$ :  $17 - i = 11, \  i = 17 - 11 = 6$. $$ P(X' \leqslant 18 - i) \leqslant 0,025$$ Le plus grand $18 - i$ tel que $P(X' \leqslant 18 - i ) \leqslant  0,025$ : $18 - i = 2,   \ i = 16$. $$R = [\![ 0 \ ; 6 ]\!] \cup [\![ 16 \ ; 18]\!]$$ 

> ### En déduire la décision. Expliquer.

$$5 \notin R \quad \text {rejet de $H_0 : \pi = 60\%$}$$ 

> ### Préciser la p-value $P_c(5)$. En déduire le degré de signification du test.


$$ P_c(5) = p_1 + p_2 $$ $$ p_1 = P(X \leqslant 5) $$ $$ p_2 = P(X \geqslant i), \\ i \text{ plus petit entier strictement supérieur à $5$ et vérifiant } P(X = i) \leqslant P(X = 5).$$ $$\begin{aligned}
&P(X = 5) = 0,0044 \\
&P(X = 17) = 0,0012 < 0,0044 \leqslant P(X = 5) \\
&P(X = 16) > P(X = 5) \\
\end{aligned} \\ p_2 = P(X \geqslant 17)$$

$$ [\![ \text{TODO : Graphe (CF BN7)} ]\!] $$

$$ \begin{aligned}
P_c (5) &=  P(X \leqslant 5) + P(X \geqslant 17) \\
&= 1 - P(X' \leqslant 18 - 5 - 1) + P(X' \leqslant 18 - 17) \\
&= 1 - P(X' \leqslant 12) + P(X' \leqslant 1) \\
&= 1 - 0,9942 + 0,0013 \approx {0,7 \%} \quad \text{(test très significatif)}
\end{aligned}$$

## Exercice 21 - Ajustement à une loi

> On souhaite comparer une distribution observée et une distribution théorique (ici uniforme de paramètre $\pi = 0,1$).
> En posant $n = 150$ et $N_1 / n$ les fréquences observées, ainsi que $\pi_i = \pi$ les probabilités attendires, on associe les effectifs observés $n_i$ et les effectifs attendus $n \pi_i = n \pi = 15$ $(0 \leqslant i \leqslant 9)$ : $$ [\![ \text{TODO : ajouter tableau} ]\!] $$ On pose : 
> * $H_0$ : le chiffre obtenu suit la loi uniforme de paramètre 0,1 ;
> * $H_1$ : le chiffre obtenu ne suite pas la loi uniforme de paramètre 0,1.
> 
> On teste $H_0$ au niveau $5\%$.
> La statistique de test (dite du $\chi^2$) utilisée est $\displaystyle D = \sum_{i = 0} ^9 {{(N_i - n \pi)^2} \over {n \pi}}$.

> ### Montrer que sous $H_0$, la vaiable associée $D$ suit une loi du $\chi^2$ à $\nu = 10 - 1 = 9$ degrés de liberté.

$$ D = \sum_0 ^9 {{(N_i - n \pi)^2} \over {n \pi}} = {{(11 - 15)^2} \over 15 } + ... + {{(18 - 15)^2} \over 15} $$ $$ D \sim \chi _{m - r - 1} ^2 \quad m : \text{nb de valeurs} \quad r : \text{nb param. estimés}$$ $$D \sim \chi_{10 - 0 - 1} ^2 = \chi_9 ^2 $$

$$ [\![ \text{TODO : Graphe CF BN8 ]\!]} $$

> ### Vérifier $n \geqslant 30$ et $\forall \  i \  (0 \leqslant i \leqslant 9), n \pi_i \geqslant 5$.

 - $n \geqslant 30$ : OK
 - $n \pi = 15 \geqslant 5$ : OK

> ### Préciser $\delta$ tel que $P(D \leqslant \delta) = 0,95$.

Table : $P(D \leqslant \delta) = 0,95 \iff \delta = 16,919$.

> ### En déduire la région de rejet au niveau $5\%$.

$$ R = [16,919 \ ; + \infty[ $$ 

> ### Peut-on rejeter l'hypothèse d'uniformité au niveau $5\%$ ?

$$ d = 5,067 \notin R$$

On ne rejete donc pas l'hypothèse.

> ### Préciser un encadrement de la p-value. En déduire le degré de signification du test.

$$ \begin{aligned}
P_c(d) &= P_{H_0} (D \geqslant d)  \\
&= 1 - P_{H_0} (D \leqslant d) \\
&= 1 - P_{H_0} (D \leqslant 5,06)
\end{aligned} $$ Table de probabilité : $$ P(D \leqslant 4,507) \leqslant P(D \leqslant 5,06) \leqslant P(D \leqslant 5,38) $$ $$ \begin{gathered}
1 - 0,200 \leqslant P_c(d) \leqslant 1-0,125 \\
0,800 \leqslant P_c(d) \leqslant 0,875
\end{gathered} \\
\text{Test non significatif.}$$

## Exercice 22 - Test d'indépendance

> Un échantillon aléatoir de $n = 200$ jeunes de 18 à 25 ans est interrogé sur la pratique d'un sport (régulière, occasionnelle, aucune) et le statut (étudiant ou non) : $$ [\![ \text{TODO : ajouter tableau} ]\!] $$ En utilisant un test du $\chi^2$ d'indépendance de niveau $\alpha = 0,05$, on souhaite savoir si la pratique d'un sport est indépendante du statut (hypothèse nulle $H_0$) ou non (hypothèse alternative $H_1$).


Étudiant : $32 + 65 + 23 = 120 = 60\%$
Non-étudiant : $11 + 40 + 29 = 80 = 40\%$
Pratique régulière : $43 \over 200$
Pratique occasionnelle : $105 \over 200$
Aucune pratique : $52 \over 200$

$200 \times {43 \over 200} \times {120 \over 200} \approx 25,8$
$$ \dots $$

Proportions : 

Statut\Pratique | Régulière | Occasionnelle | Aucune
- | - | - | -
Étudiant | $$25,8$$ | $$63,0$$ | $$31,2$$
Non-étudiant  | $$17,2$$ | $$42,0$$ | $$20,8$$

$${{(32 - 25,8)^2} \over {25,8}} + \dots + {{(29 - 20,8)^2} \over {20,8}} = d $$
